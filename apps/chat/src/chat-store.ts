...
export interface DomainResearchJobRecord {
  ...
  sessionId: string;
  domain: string;
  status: DomainResearchJobStatus;
  createdAt: string;
  updatedAt: string;
}

// A paid attempt can fail outright, which the free website path never does, so
// job status carries "failed" from schema 3 onwards.
export type DomainResearchJobStatus =
  | "queued"
  | "completed"
  | "partial"
  | "failed";
...
// Each paid provider call reports its own outcome so a partial run can say
// exactly which evidence is missing instead of being flattened to "no results".
...
function seoSnapshotFromRow(row: SeoSnapshotRow): SeoSnapshotRecord {
  const result: SeoSnapshotRecord = {
    ...
    componentStatus: JSON.parse(row.component_status_json) as Record<
      string,
      PaidComponentStatus
    >,
...
export class ChatStore {
  ...
  private migrate(): void {
    ...
    if (version < 3) {
      // Additive step for paid research. The jobs table is rebuilt only to widen
      // its status CHECK to include 'failed' (SQLite cannot alter a CHECK in
      // place); every existing row is copied across first, and no conversation
      // or business-memory data is touched.
      this.transaction(() => {
        this.database.exec(`
          DROP INDEX IF EXISTS domain_research_jobs_session;
          ...
          CREATE INDEX IF NOT EXISTS seo_snapshots_domain_captured
          ON seo_snapshots(domain, captured_at DESC);

          CREATE INDEX IF NOT EXISTS seo_snapshots_session_updated
  ...
  // Writes one paid attempt. A failed attempt is recorded in history but is
  // never allowed to carry business memory, so the last good company facts
  ...
  // Only successful or partial runs are eligible as the latest snapshot, so a
  // failed attempt never becomes the answer to "what do we know about this
  // domain".
